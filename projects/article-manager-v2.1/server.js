...
// ============ 文章分类定义 ============
const CATEGORIES = [
  { id: 'writing', name: '写作素材', icon: '✍️', color: '#0071e3' },
  { id: 'poetry', name: '古诗古文', icon: '📜', color: '#af52de' },
  { id: 'politics', name: '时政热点', icon: '🔥', color: '#ff3b30' },
  { id: 'patriotism', name: '家国情怀', icon: '🇨🇳', color: '#ff9f0a' },
  { id: 'tech', name: '科技人文', icon: '🔬', color: '#34c759' },
  { id: 'thinking', name: '思辨阅读', icon: '🧠', color: '#5856d6' }
];
...
// 获取分类统计
app.get('/api/categories/stats', requireAuth, (req, res) => {
...
app.post('/api/articles', requireAuth, upload.single('image'), (req, res) => {
  ...
  if (!category || !CATEGORIES.find(c => c.id === category)) {
    return res.json({ success: false, error: '请选择文章分类' });
  }
  const articles = readJSON(ARTICLES_FILE);
  const article = {
    id: Date.now().toString(),
...
app.put('/api/articles/:id', requireAuth, upload.single('image'), (req, res) => {
  const { title, author, content, category, removeImage } = req.body;
  const articles = readJSON(ARTICLES_FILE);
  ...
  if (category && !CATEGORIES.find(c => c.id === category)) {
    return res.json({ success: false, error: '无效的分类' });
  }
  ...
  articles[idx] = {
    ...
    author: (author || '').trim(),
    content: content || '',
    category: category || old.category || 'writing',
